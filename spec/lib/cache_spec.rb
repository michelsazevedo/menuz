...
RSpec.describe Cache do
  ...
  describe '#get' do
    context 'when the value exists' do
      ...
      let(:serialized_data) { Oj.dump(restaurant, mode: :object) }
      let(:key) { "restaurant:#{restaurant.id}" }

      before do
        allow(redis).to receive(:get).with(key).and_return(serialized_data)
        allow(Oj).to receive(:load).with(serialized_data, mode: :object).and_return(restaurant)
      end
      it 'retrieves and deserializes a value' do
        expect(Cache.instance.get(key)).to eq(restaurant)
      end
    end
  ...
  describe '#fetch' do
    ...
    context 'when the value exists' do
      ...
      it 'returns cached value if key exists' do
        expect(Cache.instance.fetch(key) { restaurant }).to eq(restaurant)
    ...
    context 'when the value does not exists' do
      before do
        ...
        allow(Oj).to receive(:dump).with(restaurant, mode: :object).and_return(serialized_data)
      end
      it 'computes and caches value if key does not exist' do
        expect(Cache.instance.fetch(key, ttl: 60) { restaurant }).to eq(restaurant)
